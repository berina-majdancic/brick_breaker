...
void Game::handle_input() {
	...
	while (SDL_PollEvent(&event)) {
		if (event.window.event == SDL_WINDOWEVENT_FOCUS_LOST) {
			SDL_SetWindowFullscreen(window_, 0); in_menu_ = true;
		}
...
void Game::render_menu()
{
	...
	std::string a;
	a = "BRICK BREAKER";
	display_text(a, window_width_ / 2, window_height_ / 4, 80, { 255,255,255 }, true);
	if (selected_menu_item_ == 0) a = !game_in_progress_ ? "> START GAME <" : "> RESUME GAME <";
	else a = !game_in_progress_ ? "START GAME" : "RESUME GAME";
	display_text(a, window_width_ / 2, window_height_ / 2, 40, { 255,255,255 }, true);
	if (selected_menu_item_ == 1) a = "> EXIT GAME <";
	else a = "EXIT GAME";
	display_text(a, window_width_ / 2, window_height_ / 2 + 50, 40, { 255,255,255 }, true);

	a = "- Berina Majdancic -";
	display_text(a, window_width_ / 2, window_height_ - 50, 30, { 255,255,255 }, true);
...
void Game::handle_menu_input()
{
	...
	if (keystate[SDL_SCANCODE_DOWN] || keystate[SDL_SCANCODE_S]) {
		if (selected_menu_item_ + 1 <= 1) selected_menu_item_++;
	}
...
void Game::display_text(const std::string& text, int x, int y, int size, SDL_Color color, bool centered)
{
	if (font_) TTF_CloseFont(font_);
	font_ = TTF_OpenFont("assets/fonts/silkscreen.ttf", size);
	if (!font_) {
		std::cerr << "Failed to load font with new size: " << TTF_GetError() << std::endl;
	}
	SDL_Surface* surface = TTF_RenderText_Solid(font_, text.c_str(), color);
	int text_width, text_height;
	if (TTF_SizeText(font_, text.c_str(), &text_width, &text_height) != 0) std::cerr << "TTF_SizeText Error: " << TTF_GetError() << std::endl;
...
void Game::quit() {
	...
	else if (font_) TTF_CloseFont(font_);
	IMG_Quit();
	TTF_Quit();
	SDL_Quit();
}